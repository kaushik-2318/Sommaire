    ...
    <section className="relative overflow-hidden bg-gray-50">
      <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8 lg:py-24">
        ...
        <div className="mb-16 text-center">
          <MotionH2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mb-4 text-xl font-bold text-rose-500 uppercase">
            How it works
          </MotionH2>
          <MotionH3 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="mx-auto max-w-2xl text-3xl font-bold">
        ...
        <div className="relative mx-auto grid max-w-6xl grid-cols-1 gap-8 md:grid-cols-3">
          {steps.map((step, idx) => (
            <MotionDiv initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: idx * 0.2 }} className="relative flex items-stretch" key={idx}>
              <StepItem {...step} />
              {idx < steps.length - 1 && (
                <MotionDiv initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.5, delay: idx * 0.2 + 0.3 }} className="absolute top-1/2 -right-4 z-10 hidden -translate-y-1/2 transform md:block">
                  <MoveRight
                    size={32}
                    strokeWidth={1}
