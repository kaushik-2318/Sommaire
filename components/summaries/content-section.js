...
const EmojiPoint = ({ point }) => {
  ...
  return (
    <MotionDiv viewport={{ once: true }} variants={itemVariants} className="group relative rounded-2xl border border-gray-500/10 bg-linear-to-br from-gray-200/[0.08] to-gray-400/[0.03] p-4 transition-all hover:shadow-lg">
...
export default function ContentSection({ points }) {
  return (
    <MotionDiv viewport={{ once: true }} variants={containerVariants} key={points.join('')} initial="hidden" animate="visible" exit="exit" className="space-y-4">
